...
export default [
  ...
  {
    title: { type: types.plain, val: '并发请求' },
    desc: [
      {
        type: types.js, val: `
          axios
  ...
  {
    title: { type: types.plain, val: '浏览器事件' },
    desc: [
      { type: types.plain, val: `
        捕获阶段
        ...
        冒泡阶段
        ` },
      { type: types.plain, val: 'onclick直接绑定的事件发生在了冒泡阶段' },
      { type: types.plain, val: 'target是真正发生事件的DOM元素，而currentTarget是当前事件发生在哪个DOM元素上' },
      { type: types.plain, val: `e.preventDefault()
        可以阻止事件的默认行为发生，默认行为是指：点击a标签就转跳到其他页面、拖拽一个图片到浏览器会自动打开、点击表单的提交按钮会提交表单等等` },
      { type: types.plain, val: `e.stopPropagation()
